...
void _qemu_init_ethernet() {
  ...
  // Configurar DNS: apuntar al gateway slirp (192.168.4.1).
  // Slirp reenviarà les queries DNS al servidor configurat a -nic dns=8.8.8.8.
  // Sense això, lwIP no sap a qui fer getaddrinfo() i la resolució de hostnames falla.
  esp_netif_dns_info_t dns_info;
  IP4_ADDR(&dns_info.ip.u_addr.ip4, 192, 168, 4, 1);
  dns_info.ip.type = ESP_IPADDR_TYPE_V4;
  esp_err_t dns_ret = esp_netif_set_dns_info(_qemu_eth_netif, ESP_NETIF_DNS_MAIN, &dns_info);
  if (dns_ret != ESP_OK) {
    Serial.printf("[QEMU-ETH] AVÍS: esp_netif_set_dns_info retorna 0x%x\n", dns_ret);
  } else {
    Serial.println("[QEMU-ETH] DNS configurat: 192.168.4.1 (slirp → 8.8.8.8)");
  }
